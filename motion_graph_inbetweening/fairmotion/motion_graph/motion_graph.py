...
class MotionGraph(object):
	...
	def inbetween(self, start_pose, end_pose, steps, epsilon, freedom, lam,
	num_comparison=3):
		"""
		...
		start_pose - the start pose (keyframe). If None, randomly choose from
		motion graph nodes.
		end_pose - the end pose (keyframe). If None, randomly choose from
		motion graph nodes.
		...
		for i in range(steps):
			...
			for path in prev_step:
				...
				min_node = None
				for curr_node in self.graph.successors(prev_node):
					# Condition to reduce search space
					start_diff = self.difference(self.graph.nodes[curr_node], self.graph.nodes[start_node], self.frames_compare, num_comparison)
					end_diff = self.difference(self.graph.nodes[curr_node], self.graph.nodes[end_node], self.frames_compare, num_comparison)
					...
					if  (start_diff*(i/steps) + end_diff*(1-i/steps) <= diff): # TODO: Check implementation (priority 2)
					   curr_step.append(path + [curr_node])
					   appended = True
				if not appended:
					if self.verbose:
	...
	def create_random_motion(self, length, start_node=None, blend=True):
		"""
