...
def compare_and_connect_edge(
    ...
    logging.info(f'Node {node_id} done')
    return res
...
class MotionGraph(object):
    ...
    def difference(self, node_A, node_B, frames_compare, num_comparison):
        ...
        for k in range(
            ...
            vel = self.motions[motion_idx_A].get_velocity_by_frame(frames_A + k)
            pose_j = self.motions[motion_idx_B].get_pose_by_frame(frames_B + k)
            vel_j = self.motions[motion_idx_B].get_velocity_by_frame(frames_B + k)
            if k == 0:
                T_ref = pose.get_facing_transform()
                T_ref_j = pose_j.get_facing_transform()
            diff_pose += similarity.pose_similarity(
                pose, pose_j, vel, vel_j, self.w_joint_pos, self.w_joint_vel, self.w_joints
            )
            diff_root_ee += similarity.root_ee_similarity(
                pose,
                pose_j,
    ...
    def inbetween_bfs(self, steps, epsilon, freedom, lam, start_node=None, end_node=None,
    start_pose=None, end_pose=None, new=True, num_comparison=3):
        """
        This funtion generate a transition motion from the given motion graph,
        a start and end pose, and various tuning parameters by traversing the
        graph.

        start_pose - the start pose (keyframe). If None, and no start node provided,
        randomly choose from motion graph nodes.
        end_pose - the end pose (keyframe). If None, and no end node provided,
        randomly choose from motion graph nodes.
        start_node - the start pose (node index). If None, and no start pose provided,
        randomly choose from motion graph nodes.
        end_node - the end pose (node index). If None, and no end pose provided,
        randomly choose from motion graph nodes.

        steps - the number of steps for the transition motion between the start
        and end pose. If steps <= 0, number of steps will vary depending on
        graph traversal.
        epsilon - biggest amount of difference acceptable to consider poses as
        the same.
        freedom - how different a pose in the transition can be from the given poses
        lam - freedom decay factor
        """
        nodes = list(self.graph.nodes)

        if start_pose:
            start_node = mg.find_similar_node(start_pose) # TODO: Implement (priority 3)
        elif start_node:
        ...
        if end_pose:
            end_node = mg.find_similar_node(end_pose)
        elif end_node:
            end_node = nodes[end_node]
        else:
            end_node = random.choice(nodes)

        prev_step = [[start_node]]
        for i in range(steps):
            curr_step = []
            for path in prev_step:
                prev_node = path[-1]
                if self.graph.out_degree(prev_node) == 0:
                    if self.verbose:
                        logging.info("Dead-end exists in the graph!")
                    pass
                ...
                min_node = None
                for cur_node in self.graph.successors(prev_node):
                    if new and (
                    self.graph.nodes[prev_node]["motion_idx"] == self.graph.nodes[cur_node]["motion_idx"] - 1 and
                    self.graph.nodes[prev_node]["frame_end"] != self.graph.nodes[cur_node]["frame_start"]):
                        continue
                    # Condition to reduce search space
                    start_diff = self.difference(self.graph.nodes[cur_node], self.graph.nodes[start_node], self.frames_compare, num_comparison)
                    end_diff = self.difference(self.graph.nodes[cur_node], self.graph.nodes[end_node], self.frames_compare, num_comparison)
                    diff = max(freedom, epsilon)
                    freedom = freedom * lam
                    if not appended and start_diff*(i/steps) + end_diff*(1-i/steps) < min_so_far:
                        min_so_far = start_diff*(i/steps) + end_diff*(1-i/steps)
                        min_node = cur_node
                    if  (start_diff*(i/steps) + end_diff*(1-i/steps) <= diff): # TODO: Check implementation (priority 2)
                       curr_step.append(path + [cur_node])
                       appended = True
                if not appended:
                    if self.verbose:
                        logging.info('No node found that is less than difference.')
                    if min_node:
                        curr_step.append(path + [min_node])
            prev_step = curr_step

        transitions = {tuple(path):self.difference(self.graph.nodes[path[-1]], self.graph.nodes[end_node], self.frames_compare, num_comparison) for path in prev_step}
        shorten_transitions = [list(path) for path in sorted(transitions, key=transitions.__getitem__) if transitions[tuple(path)] < epsilon]
        if len(shorten_transitions) > 0:
            transitions = shorten_transitions
        else:
            transitions = [list(path) for path in sorted(transitions, key=transitions.__getitem__)]
            if len(transitions) == 0:
                return None, None
        if self.verbose:
            logging.info(f'# transitions: {len(transitions)}')
            logging.info(f'Nodes: {transitions[0]}')
        return self.motionify(transitions[0]), transitions

    def replace(self, goal, candidate, chosen, new=False, num_comparison=3):
        cand_diff = self.difference(self.graph.nodes[goal], self.graph.nodes[candidate], self.frames_compare, num_comparison)
        chose_diff = self.difference(self.graph.nodes[goal], self.graph.nodes[chosen], self.frames_compare, num_comparison)
        pr = chose_diff / (cand_diff+chose_diff)
        return random.random() < pr

    def inbetween_next_node(self, start_node, end_pose, steps, new=False, num_comparison=3):
        chosen = None
        error = None
        # print(list(self.graph.successors(start_node)))
        for cur_node in self.graph.successors(start_node):
            if steps == 0 and (chosen == None or self.replace(end_pose, cur_node, chosen[-1], new=new, num_comparison=num_comparison)):
                chosen = [cur_node]
            else:
                next, error = self.inbetween_next_node(cur_node, end_pose, steps-1, new=new, num_comparison=num_comparison)
                candidate = [start_node] + next
                if chosen == None or self.replace(end_pose, candidate[-1], chosen[-1], new=new, num_comparison=num_comparison):
                    chosen = candidate
        if not chosen:
            chosen = [start_node]
            error = f'No node chosen. Number of successors for {start_node}: {len(list(self.graph.successors(start_node)))}'
            logging.info(error)
        return chosen, error

    def inbetween(self, steps, start_node=None, end_node=None, start_pose=None, end_pose=None, new=False, num_comparison=3):
        """
        This funtion generate a transition motion from the given motion graph,
        a start and end pose, and various tuning parameters by traversing the
        graph.
        ...
        if start_pose:
            ...
            #start_node = mg.find_similar_node(start_pose) # TODO: Implement
        elif start_node:
            start_node = nodes[start_node]
        else:
            start_node = random.choice(nodes)
        ...
        if end_pose:
            raise Exception("Not supported yet")
            #end_node = mg.find_similar_node(end_pose) # TODO: Implement
        elif end_node:
            end_node = nodes[end_node]
        else:
            end_node = random.choice(nodes)

        transitions, error = self.inbetween_next_node(start_node, end_node, steps, new=new, num_comparison=num_comparison)

        diff = None#self.difference(self.graph.nodes[transitions[-1]], self.graph.nodes[end_node], self.frames_compare, num_comparison)

        return self.motionify([153, 154, 155, 156, 157, 158]), transitions, start_node, end_node, diff, error

    def create_random_motion(self, length, start_node=None, new=True, blend=True):
        """
            ...
            while t_processed < length:
                ...
                if new:
                    for successor in successors:
                        if (self.graph.nodes[successor]["motion_idx"] != motion_idx + 1 and
                            self.graph.nodes[successor]["frame_start"] != frame_end):
                            cur_node = successor
                            break
    ...
    def reduce(self, method="scc", num_component=1):
        ...
        logging.info(f"Reduced graph ({nn} nodes / {ne} edges)")
