...
class Pose(object):
    ...
    def get_transform(self, key, local):
        ...
        if local:
            ...
            # TODO:
            # Not using this for root. Using local=True instead.
            # xform_from_parent_joint has the position but so does
            # self.data[root] so position gets summed.
            # T = np.dot(
            #     joint.xform_from_parent_joint,
            #     self.data[skel.get_index_joint(joint)],
            # ) # Original code
            T = self.data[skel.get_index_joint(joint)]
    ...
    def get_root_transform(self):
        ...
        # TODO: Set local=False to True. Check if that's ok.
        return self.get_transform(root_idx, local=False)

    def set_root_transform(self, T, local):
        root_idx = self.skel.get_index_joint(self.skel.root_joint)
        self.set_transform(root_idx, T, local=False)
